...
TTS_PREFIX = "ftm"  # cache key prefix to keep separate from other reels
...
# --------------------------------------------------------------------------- #
# Beats. (slug, dur, chord_key, chip_label, footage_spec)
# Three cuts come from the SAME source (170328 — Wigella's stats speech).
# Same speech, three different argumentative beats — natural audio cohesion.
# --------------------------------------------------------------------------- #
...
def render_cta_chrome(well_transparent=True):
    ...
    tag_bbox = draw_centered_text(
        draw, CTA_HEADLINE, font(72, bold=True), W // 2, tag_top,
        fill=(*C["white"], 255),
        shadow_offset=(4, 4),
    ...
    sub_bbox = draw_centered_text(
        draw, CTA_SUBHEAD, font(48, bold=True), W // 2, sub_top,
        fill=(*C["white"], 255))
...
def synthesize_narration(env):
    ...
    for line in NARRATION_LINES:
        cache = TTS_CACHE / f"{TTS_PREFIX}_{line['slug']}.mp3"
        if not cache.exists():
...
def measure_tts_duration(slug):
    try:
        ...
        return 0.0
    p = TTS_CACHE / f"{TTS_PREFIX}_{slug}.mp3"
    if not p.exists():
        return 0.0
    try:
...
def build_caption_events():
    """If a beat's spec has `caption_lines`, those override Whisper. Each
    entry is (start_in_beat, end_in_beat, text). For the CTA beat, the
    synth VO line is used and timed to its measured TTS duration."""
    events = []
            ...
            for line in NARRATION_LINES:
                ...
                if vo_dur > 0.0:
                    ...
                    events.append({"start": start,
                                   ...
                                   "text":  line["text"]})
            continue
        if spec.get("caption_lines"):
            for s_start, s_end, text in spec["caption_lines"]:
...
def main():
    print("Building MPC Follow the Money (30s receipts reel)...")
