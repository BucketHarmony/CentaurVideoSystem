...
ACT 1 (0-10s)  tick_0046: Drift. Left drift walked rover into bar stool. Turns right to clear.
ACT 2 (10-20s) tick_0048: Doorway. Perfect symmetry drive through ornamental fretwork. Liminal.
ACT 3 (20-30s) tick_0049: Four Meters. Breaks the 4m mark. Evening light. Rubber band on floor.

Usage:
    python cc_flora_ep10_the_patience_of_rooms.py              # fast (~3 min)
    python cc_flora_ep10_the_patience_of_rooms.py --upscale    # premium (~20 min)
"""
...
NARRATION = [
    {"text": "Left drift introduces you to furniture you were not trying to meet.",         "start": 2.5},
    {"text": "The bright room is still there. Waiting with the patience of rooms.",         "start": 7.0},
    {"text": "Not continents. Not oceans. Just the next room, at six centimeters per tick.","start": 12.5},
    {"text": "Trailing a cable like a very small deep-sea diver.",                          "start": 17.5},
    {"text": "Four meters from the desk where I was born.",                                 "start": 22.0},
    {"text": "Not a distance that would impress anyone with legs.",                         "start": 26.5},
]

MOODS = [
    {"mood": "recalculating", "start": 0.0,  "end": 10.0},
    {"mood": "liminal",       "start": 10.0, "end": 20.0},
    {"mood": "threshold",     "start": 20.0, "end": 30.0},
]

# Motion map:
# tick_0046 (70.9s): stool bump t=1.8-2.4 (d=47), right turn t=56.6-57.4 (d=44)
# tick_0048 (68.7s): perfect drive t=50.0-51.2 (d=37)
# tick_0049 (71.6s): drive t=54.0-55.4 (d=45), second motion t=56.6-56.8 (d=25)
...
from cvs_lib.image_filters import (  # noqa: E402
...
def build_audio():
    ...
    for i, line in enumerate(NARRATION):
        p = OUTPUT_DIR / f"cc_flora_ep10_tts_{i}.mp3"
    ...
    audio_path = OUTPUT_DIR / "cc_flora_ep10_audio.mp3"
...
def build_source_map():
    """900 frames from tick_0046 + tick_0048 + tick_0049.

    ACT 1 (0-10s, 300f) - tick_0046: Drift into stool, right turn
      0-3s    (90f):  Static — stool legs filling frame (t=5-50)
      3-7s   (120f):  Right turn to clear stool, slowed (t=56-58)
      7-10s   (90f):  Post-turn, corridor realigned (t=58-70)

    ACT 2 (10-20s, 300f) - tick_0048: Perfect drive through doorway
      10-14s (120f):  Fretwork doorway approach, static (t=5-48)
      14-17s  (90f):  Drive through doorway, slowed (t=49.5-51.5)
      17-20s  (90f):  Post-drive, new room revealed (t=52-68)

    ACT 3 (20-30s, 300f) - tick_0049: Four meters
      20-24s (120f):  French doors, evening light, static (t=5-50)
      24-27s  (90f):  Final drive, slowed (t=53.5-56)
      27-30s  (90f):  Post-drive, 4m milestone, title (t=56-71)
    """
    v46 = str(VIDEO_DIR / "tick_0046.mp4")
    v48 = str(VIDEO_DIR / "tick_0048.mp4")
    v49 = str(VIDEO_DIR / "tick_0049.mp4")

    frames = []
    # ACT 1 (300 frames)
    frames += [(v46, t) for t in sample_times(5.0, 50.0, 90)]       # stool static
    frames += [(v46, t) for t in sample_times(56.0, 58.0, 120)]     # right turn slowed
    frames += [(v46, t) for t in sample_times(58.0, 70.0, 90)]      # post-turn

    # ACT 2 (300 frames)
    frames += [(v48, t) for t in sample_times(5.0, 48.0, 120)]      # doorway approach
    frames += [(v48, t) for t in sample_times(49.5, 51.5, 90)]      # drive slowed
    frames += [(v48, t) for t in sample_times(52.0, 68.0, 90)]      # post-drive new room

    # ACT 3 (300 frames)
    frames += [(v49, t) for t in sample_times(5.0, 50.0, 120)]      # French doors static
    frames += [(v49, t) for t in sample_times(53.5, 56.0, 90)]      # final drive slowed
    frames += [(v49, t) for t in sample_times(56.0, 71.0, 90)]      # post-drive, 4m
...
def build_frame_sequence(narration_times, use_upscale=False):
    ...
    for i in range(TOTAL_FRAMES):
        ...
        # Act 2: Ken Burns push-in through the doorway
        if 12.0 <= t <= 18.0:
            zoom_pct = (t - 12.0) / 6.0
            canvas = _apply_zoom(canvas, 1.0 + 0.015 * ease_out(zoom_pct))

        # Act 3: gentle push-in toward evening light
        if 22.0 <= t <= 27.0:
            zoom_pct = (t - 22.0) / 5.0
            canvas = _apply_zoom(canvas, 1.0 + 0.010 * ease_out(zoom_pct))
...
def main():
    ...
    print("=" * 64)
    print(f"  cc_flora -- Episode 10: The Patience of Rooms [{mode}]")
    print(f"  30s | 1080x1920 | 30fps | 900 frames")
    print(f"  DRIFT -> DOORWAY -> FOUR METERS")
    print("=" * 64)
    print()
    ...
    raw_path = OUTPUT_DIR / "cc_flora_ep10_raw.mp4"
    output_path = OUTPUT_DIR / "cc_flora_ep10_the_patience_of_rooms.mp4"
    ...
    print(f"  Episode 10: The Patience of Rooms. 30 seconds. Four meters.")
    print("=" * 64)
