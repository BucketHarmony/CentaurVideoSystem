#!/usr/bin/env python3
"""
CVS — "Beetle" — 30 seconds. The upside-down ticks.
0-3s: The fall (tick 277). 3-30s: Looking around from its back (tick 280).
"""
...
def gen_audio(dur, motion, fall_dur):
    ...
    chimes=np.zeros(n)
    for ct2,freq in [(ps+3,587.33),(ps+10,698.46),(ps+18,880.0)]:
...
def main():
    print("=" * 50)
    print("  BEETLE — 30 seconds. Fall + upside down.")
    print("=" * 50)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    td = OUTPUT_DIR / "beetle_tts"; td.mkdir(exist_ok=True)

    v277 = KOMBUCHA_DIR / "video" / "web" / "tick_0277.mp4"
    v280 = KOMBUCHA_DIR / "video" / "web" / "tick_0280.mp4"

    # ── TTS: two tight lines ──
    print("\n  TTS...")
    lines = [
        ("Two ticks staring at a ceiling light like a moth with wheels. The revolution will not be televised if the camera is pointed at the ceiling.",
         280),
        ("Three ticks of increasingly desperate driving into nothing, like a beetle flipped on its back, wheels spinning against air.",
         281),
    ]
    ...
    ct = narr_start
    gap = 0.6
    for i, (text, tick) in enumerate(lines):
    ...
    if any(tc[0] is not None for tc in tts_clips):
        ...
        audio = bed
    ap = OUTPUT_DIR / "beetle_audio.wav"
    write_wav(ap, audio)
    ...
    # Tick 280: the desperate upside-down driving sequence (most motion at 150s)
    upside_needed = total_frames - len(fall_frames)
    upside_frames = load_sm(v280, 90, upside_needed, speed=0.5)
    print(f"    Upside down: {len(upside_frames)}")

    # ── Render ──
    print(f"\n  Rendering {total_frames} frames...")
    fd = OUTPUT_DIR / "beetle_frames"; fd.mkdir(exist_ok=True)
    ...
    for fi in range(total_frames):
        ...
        if fi < fall_n:
            ...
            img = gf(upside_frames[min(fi-fall_n, len(upside_frames)-1)])
            d = ImageDraw.Draw(img)
            d.text((60, 170), "tick 0280", fill=MUTED, font=ftk)
            # Mood pill
            rgba=img.convert("RGBA"); ov=Image.new("RGBA",img.size,(0,0,0,0)); od=ImageDraw.Draw(ov)
        ...
        if gt > 2.5:
            ta = min((gt-2.5)*3, 1.0)
            img = pill(img, "BEETLE", 100, ft, tuple(int(c*ta) for c in CREAM))
        ...
        if (fi+1) % 90 == 0:
            print(f"    [{(fi+1)/total_frames*100:5.1f}%] Frame {fi+1}/{total_frames}")

    # ── Encode ──
    print("  Encoding...")
    out = OUTPUT_DIR / "beetle_30sec.mp4"
    subprocess.run([
