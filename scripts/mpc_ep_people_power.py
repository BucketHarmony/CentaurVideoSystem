...
def render_beat_chrome(slug, chip_label, well_transparent=True):
    """Banner + (optional) proof chip over a transparent well. Pass
    chip_label=None to suppress the chip — montage cuts use this so the
    only chrome on screen is the brand banner."""
...
def harmonic_hum():
    """Thinner bed for this reel — chant audio is doing the heavy lifting,
    the synth pad just glues cuts together. Mixed at lower level downstream."""
...
# --------------------------------------------------------------------------- #
# Captions — only the synth-VO line at CTA. All other beats explicitly
# pass caption_lines=[] to suppress Whisper auto-pulls.
# --------------------------------------------------------------------------- #
            ...
            for line in NARRATION_LINES:
                ...
                if vo_dur > 0.0:
                    ...
                    events.append({"start": start,
                                   ...
                                   "text":  line["text"]})
            continue
        # Visual cuts use caption_lines=[] (explicit empty) — no captions.
